...
'/* Page 3 inline styles (status badge rendered via template component) */',
...
'      <span class="u-pill &P3_STATUS_UT_CLASS."><span class="u-pill-label">&P3_CURRENT_STATUS.</span></span>',
'    </div>',
'  </div>',
...
wwv_flow_imp_page.create_page_item(
 p_id=>wwv_flow_imp.id(13300017000000027)
,p_name=>'P3_STATUS_CATEGORY'
,p_item_sequence=>85
,p_item_plug_id=>wwv_flow_imp.id(13300002000000002)
,p_display_as=>'NATIVE_HIDDEN'
,p_attributes=>wwv_flow_t_plugin_attributes(wwv_flow_t_varchar2(
...
'  l_status_category VARCHAR2(30);',
'  l_status_ut_class VARCHAR2(30);',
'BEGIN',
...
'',
'  -- Status classification inline (avoid package dependency)',
'  -- Order matters: check denied first ("NOT APPROVED" before "APPROVED"),',
'  -- then pending/processing ("APPROVAL PENDING" before "APPROVED"),',
'  -- then approved/card-production last among the top categories.',
'  l_status_category := CASE',
'    WHEN l_raw_status IS NULL THEN ''unknown''',
'    -- 1. Denied / Negative outcomes (must precede approved)',
'    WHEN UPPER(l_raw_status) LIKE ''%NOT APPROVED%''',
'      OR UPPER(l_raw_status) LIKE ''%DENIED%''',
'      OR UPPER(l_raw_status) LIKE ''%REJECT%''',
'      OR UPPER(l_raw_status) LIKE ''%TERMINAT%''',
'      OR UPPER(l_raw_status) LIKE ''%WITHDRAWN%''',
'      OR UPPER(l_raw_status) LIKE ''%REVOKED%'' THEN ''denied''',
'    -- 2. Pending / In Progress (must precede approved to catch "APPROVAL PENDING")',
...
'      OR UPPER(l_raw_status) LIKE ''%SCHEDULED%'' THEN ''pending''',
'    -- 3. Request for Evidence',
'    WHEN UPPER(l_raw_status) LIKE ''%EVIDENCE%''',
'      OR UPPER(l_raw_status) LIKE ''%RFE%'' THEN ''rfe''',
'    -- 4. Approved / Positive outcomes (after pending so compound phrases resolve correctly)',
'    WHEN UPPER(l_raw_status) LIKE ''%APPROVED%''',
'      OR UPPER(l_raw_status) LIKE ''%CARD WAS PRODUCED%''',
'      OR UPPER(l_raw_status) LIKE ''%CARD IS BEING PRODUCED%''',
'      OR UPPER(l_raw_status) LIKE ''%CARD WAS DELIVERED%''',
'      OR UPPER(l_raw_status) LIKE ''%CARD WAS MAILED%''',
'      OR UPPER(l_raw_status) LIKE ''%CARD WAS PICKED UP%''',
'      OR UPPER(l_raw_status) LIKE ''%OATH CEREMONY%''',
'      OR UPPER(l_raw_status) LIKE ''%WELCOME NOTICE%'' THEN ''approved''',
'    -- 5. Received / Accepted',
'    WHEN UPPER(l_raw_status) LIKE ''%RECEIVED%''',
'      OR UPPER(l_raw_status) LIKE ''%ACCEPTED%''',
'      OR UPPER(l_raw_status) LIKE ''%FEE WAS%'' THEN ''received''',
'    -- 6. Transferred',
'    WHEN UPPER(l_raw_status) LIKE ''%TRANSFER%''',
'      OR UPPER(l_raw_status) LIKE ''%RELOCATED%''',
'      OR UPPER(l_raw_status) LIKE ''%SENT TO%'' THEN ''transferred''',
'    ELSE ''unknown''',
'  END;',
'  l_status_ut_class := CASE l_status_category',
...
'    ELSE ''u-color-7''',
'  END;',
'  :P3_STATUS_CATEGORY := l_status_category;',
'  :P3_STATUS_UT_CLASS := l_status_ut_class;',
'',
'  -- Active display',
