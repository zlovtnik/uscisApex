...
# Uploads static files to APEX application using SQLcl
# ==============================================================================
...
SQL_SCRIPT="${SCRIPT_DIR}/upload_inline.sql"

show_help() {
    echo "Usage: $(basename "$0") [OPTIONS]"
    echo ""
    echo "Uploads static files to APEX application using SQLcl saved connections."
    echo ""
    echo "Options:"
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}       Upload Static Files              ${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
.timeline-item:hover {
  ...
  box-shadow: 0 8px 30px rgba(0, 0, 0, 0.15);
    ...
    wwv_flow_imp.create_app_static_file(
        ...
        p_file_content => utl_raw.cast_to_raw(l_css_content)
    );
...
if [ $SQLCL_EXIT_CODE -eq 0 ]; then
    ...
    echo -e "${YELLOW}Don't forget to refresh your APEX application to see the changes.${NC}"
else
    echo ""
